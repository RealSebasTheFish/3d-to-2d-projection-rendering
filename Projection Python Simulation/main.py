...
while not pressedExit:
    ...
    if origin != [-1, -1]:
        x_axis = map_point(Point(100, 0, 0), camera)
        y_axis = map_point(Point(0, 100, 0), camera)
        z_axis = map_point(Point(0, 0, 100), camera)
        if x_axis != [-1, -1]:
    ...
    for event in pygame.event.get():
        ...
        # Pan left (z)
        if keys[pygame.K_z]:
            camera.set_pan(camera.pan + rotate_sens)
        ...
        # Tilt up (c)
        if keys[pygame.K_c]:
            target_tilt = camera.tilt + rotate_sens
