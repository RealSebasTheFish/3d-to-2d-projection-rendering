...
def map_point(world, cam):
    ...
    if rel_x < 0.1 or rel_x > 1000:
        # Invalid depth or out of range (clipping)
        return [-1, -1]

    print("(" + str(rel_x) + ", " + str(rel_y) + ", " + str(rel_z) + ")")

    # Convert to screen coordinates using similar triangles
    #print(math.tan(cam.fov/2.0))
    ...
    x_map = max_x - (x*max_x/2.0 + max_x/2.0)
    y_map = max_y - (y*max_y/2.0 + max_y/2.0)
